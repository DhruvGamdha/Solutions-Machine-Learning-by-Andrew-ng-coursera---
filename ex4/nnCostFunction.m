...
sig2 = sigmoid(Theta2*(sig1));

% sig2 = 10 x m matrix

binaryY = zeros(num_labels,m);
...
J = (-1/m)*sum(sum(binaryY.*log(sig2) + (1-binaryY).*log(1-sig2)));
...
temp2(:,1) = Theta2(:,1); 
Theta1_grad = Theta1_grad + (lambda/m)*(Theta1-temp1);  
Theta2_grad = DEL2/m;
Theta2_grad = Theta2_grad + (lambda/m)*(Theta2-temp2);  
% -------------------------------------------------------------
